...
const FormattedTextOverlay = ({ content, formattedSections, style }: { 
  ...
  return (
    <View style={[styles.formattedTextOverlay, style]}>
      <Text style={{ color: style.color, fontSize: style.fontSize, lineHeight: style.lineHeight }}>
        {formattedChars.map((item, index) => (
          <Text key={index} style={item.style}>
            {item.char === ' ' ? '\u00A0' : item.char}
          </Text>
...
export default function NotesScreen() {
  ...
  const openReadModal = (note: Note) => {
    ...
    // No need to parse content here since FormattedText component will handle that
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: isDark ? '#000000' : '#F8FAFC' }]}>
      ...
      >
        <SafeAreaView style={[styles.container, { backgroundColor: isDark ? '#000000' : '#F8FAFC' }]}>
          <View style={[styles.readModalHeader, { borderBottomColor: isDark ? '#333333' : '#E2E8F0' }]}>
            <TouchableOpacity
              style={styles.backButton}
              onPress={() => setIsReadModalVisible(false)}
            >
            ...
            <TouchableOpacity
              style={[styles.editButton, { backgroundColor: isDark ? '#333333' : '#EDF2F7' }]}
              onPress={() => {
                setIsReadModalVisible(false);
                // Small delay to prevent UI glitches
                setTimeout(() => openEditModal(currentNote), 100);
              }}
            >
          ...
          <ScrollView style={styles.readModalContent}>
            <Text style={[styles.readModalTitle, { color: isDark ? '#FFFFFF' : '#000000' }]}>
              {currentNote?.title}
            </Text>
            
            <View style={styles.readModalDateContainer}>
              <Text style={[styles.readModalDate, { color: isDark ? '#666666' : '#94A3B8' }]}>
                {currentNote ? formatDate(currentNote.updatedAt) : ''}
              </Text>
            </View>
            
            <View style={styles.readModalTextContainer}>
              {currentNote && (
                <FormattedText 
                  content={currentNote.content}
                  baseStyle={[styles.readModalText, { color: isDark ? '#FFFFFF' : '#000000' }]}
                />
              )}
            </View>
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  readModalDateContainer: {
    marginBottom: 16,
  },
  readModalDate: {
    fontSize: 14,
  },
  readModalTextContainer: {
    paddingBottom: 32,
  },
  readModalText: {
    fontSize: 16,
